...
public class TransactionRepositoryEntityMap implements TransactionRepository {

    private Map<Integer, Transaction> transactions = new HashMap<>();

    private static final Logger LOGGER = LoggerFactory.getLogger(TransactionRepositoryEntityMap.class);
    ...
    public Collection<Transaction> getTransactionByBill(Bill bill) {
        ...
        for(Map.Entry<Integer, Transaction> entry: transactions.entrySet()) {
          if (entry.getValue().getBill().equals(bill)) {returnTr.add(entry.getValue());}
        }
        return returnTr;
    }
    ...
    public Integer getSummByBill(Bill bill) {
        Integer sum=0;
        for(Map.Entry<Integer, Transaction> entry: transactions.entrySet()) {
            if (entry.getValue().getBill().equals(bill)) {sum=sum + entry.getValue().getSum().intValue();}
        }
        return sum;
    }
